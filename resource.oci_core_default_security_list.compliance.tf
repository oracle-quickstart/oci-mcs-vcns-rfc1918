resource "oci_core_default_security_list" "compliance" {
  count                      = var.vcn_compliance_enable == true ? 1 : 0
  manage_default_resource_id = oci_core_vcn.compliance[0].default_security_list_id
  ...
  ingress_security_rules {
    ...
    protocol    = "1"
    source      = oci_core_vcn.compliance[0].cidr_block
    source_type = "CIDR_BLOCK"
    stateless   = "false"
  }
  ingress_security_rules {
